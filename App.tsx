        ...
        <div className="flex-1 flex flex-col bg-transparent relative min-w-0 z-10 h-full overflow-hidden">
            ...
            <div className={`flex flex-col h-full ${currentView === 'todos' ? '' : 'view-hidden'}`}>
                <MainToolbar
                  ...
                  setIsGroupedByDay={setIsGroupedByDay}
                />
                      ...
                      {filteredTodos.length === 0 && processingTodos.length === 0 ? (
                        ...
                        <div className="space-y-3 pb-24">
                           ...
                           {processingTodos.map(todo => (
                               <TodoItem key={todo.id} todo={todo} onToggle={onToggleTodo} onDelete={onDeleteTodo} onUpdate={handleUpdateTodo} focusedTodoId={focusedTodoId} setFocusedTodoId={setFocusedTodoId} />
                           ))}
                           ...
                           {isGroupedByDay ? groupedTodos.map(group => (
                             <div key={group.title} className="space-y-3">
                               ...
                               {group.tasks.map(todo => <TodoItem key={todo.id} todo={todo} onToggle={onToggleTodo} onDelete={onDeleteTodo} onUpdate={handleUpdateTodo} focusedTodoId={focusedTodoId} setFocusedTodoId={setFocusedTodoId} />)}
                             </div>
                           )) : filteredTodos.map(todo => <TodoItem key={todo.id} todo={todo} onToggle={onToggleTodo} onDelete={onDeleteTodo} onUpdate={handleUpdateTodo} focusedTodoId={focusedTodoId} setFocusedTodoId={setFocusedTodoId} />)}
                        </div>
                      )}
                    </div>
